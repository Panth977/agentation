<script lang="ts">
  let {
    size = 24,
    state = "idle",
  }: { size?: number; state?: "idle" | "sending" | "sent" | "failed" } =
    $props();
...
<svg width={size} height={size} viewBox="0 0 24 24" fill="none">
  ...
  <g class="iconStateFast {showCheck ? 'visibleScaled' : 'hiddenScaled'}">
    <path
  ...
  <g class="iconStateFast {showError ? 'visibleScaled' : 'hiddenScaled'}">
    <path
      d="M12 20C7.58172 20 4 16.4182 4 12C4 7.58172 7.58172 4 12 4C16.4182 4 20 7.58172 20 12C20 16.4182 16.4182 20 12 20Z"
