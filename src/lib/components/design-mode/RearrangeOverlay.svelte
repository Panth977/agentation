<script lang="ts" module>
  ...
  export type RearrangeOverlayProps = {
...
<script lang="ts">
  ...
  let editHadNoteRef = $state(false);
  ...
  const lastNoteTextRef = new Map<string, string>();
  let hoverHighlight = $state<{ x: number; y: number; w: number; h: number } | null>(null);
  ...
  const seenGhostIdsRef = new Set<string>();
  // Track which action (move/resize) happened first per section, for badge ordering
  const firstActionRef = new Map<string, "move" | "resize">();
  // Live drag/resize positions for connector lines during interaction
  let dragPositions = $state<Map<string, Rect>>(new Map());
  // Exiting connectors: sections that returned to original position, animating out
  let exitingConnectors = $state<Map<string, { orig: Rect; target: Rect; isFixed?: boolean }>>(new Map());
  let prevChangedIdsRef = new Set<string>();
  // Track last known currentRect for each changed section (for exit animation)
  const lastChangedRectsRef = new Map<string, { currentRect: Rect; originalRect: Rect; isFixed?: boolean }>();

  // Clear selection when blank canvas is toggled on
  $effect(() => {
    if (blankCanvas) selectedIds = new Set();
  });
  ...
  $effect(() => {
    if (!outlinesReady) {
      const timer = originalSetTimeout(() => (outlinesReady = true), 380);
  ...
  $effect(() => {
    // re-track when sections change (capturedSelectors derives from it)
    sections;
    const handleMouseMove = (e: MouseEvent) => {
      if (interactionRef) { hoverHighlight = null; return; }

      const el = document.elementFromPoint(e.clientX, e.clientY) as HTMLElement | null;
      if (!el) { hoverHighlight = null; return; }
      if (el.closest("[data-feedback-toolbar]")) { hoverHighlight = null; return; }
      if (el.closest("[data-design-placement]")) { hoverHighlight = null; return; }
      if (el.closest("[data-annotation-popup]")) { hoverHighlight = null; return; }

      const target = pickTarget(el);
      if (!target) { hoverHighlight = null; return; }
      ...
      hoverHighlight = { x: rect.x, y: rect.y, w: rect.width, h: rect.height };
    };
  ...
  $effect(() => {
    // deps: sections, rearrangeState, onChange
    ...
    const handleMouseDown = (e: MouseEvent) => {
      ...
      if (target && !alreadyCaptured) {
        ...
        selectedIds = newIds;
        onSelectionChange?.(newIds, isShift);
        hoverHighlight = null;
        ...
        const onMove = (ev: MouseEvent) => {
          ...
          dragPositions = new Map([[section.id, { x: startPos.x + snappedDx, y: startPos.y + snappedDy, width: section.currentRect.width, height: section.currentRect.height }]]);
  ...
  $effect(() => {
    // deps: selectedIds, sections, rearrangeState, onChange
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if ((e.key === "Backspace" || e.key === "Delete") && selectedIds.size > 0) {
        ...
        { const next = new Set(exitingIds); for (const id of idsToDelete) next.add(id); exitingIds = next; }
        selectedIds = new Set();
        originalSetTimeout(() => {
          ...
          { const next = new Set(exitingIds); for (const id of idsToDelete) next.delete(id); exitingIds = next; }
        }, 180);
        return;
      }
  ...
  function handleOutlineMouseDown(e: MouseEvent, id: string) {
    if (e.button !== 0) return;
    ...
    if (changed) onSelectionChange?.(newSelected, !!(e.shiftKey || e.metaKey || e.ctrlKey));
    ...
    const dragEls = new Map<string, { outlineEl: HTMLElement | null; curW: number; curH: number }>();
    for (const s of sections) {
      if (newSelected.has(s.id)) {
        const outlineEl = document.querySelector(`[data-rearrange-section="${s.id}"]`) as HTMLElement | null;
        dragEls.set(s.id, { outlineEl, curW: s.currentRect.width, curH: s.currentRect.height });
      }
    }
    ...
    const onMove = (ev: MouseEvent) => {
      ...
      const livePos = new Map<string, Rect>();
      for (const [id2, { curW, curH }] of dragEls) {
        const start = startPositions.get(id2);
        if (start) {
  ...
  function handleResizeMouseDown(e: MouseEvent, id: string, dir: HandleDir) {
    ...
    const onMove = (ev: MouseEvent) => {
      ...
      dragPositions = new Map([[id, lastRect]]);
    };
  ...
  function handleDelete(id: string) {
    { const next = new Set(exitingIds); next.add(id); exitingIds = next; }
    { const next = new Set(selectedIds); next.delete(id); selectedIds = next; }
    originalSetTimeout(() => {
      const rs = rearrangeStateRef;
      onChange({
        ...rs,
      ...
      { const next = new Set(exitingIds); next.delete(id); exitingIds = next; }
    }, 180);
  }

  function hasChanged(s: DetectedSection): boolean {
    const o = s.originalRect, c = s.currentRect;
    return Math.abs(o.x - c.x) > 1 || Math.abs(o.y - c.y) > 1 || Math.abs(o.width - c.width) > 1 || Math.abs(o.height - c.height) > 1;
  }

  function isMoved(s: DetectedSection): boolean {
    const o = s.originalRect, c = s.currentRect;
    return Math.abs(o.x - c.x) > 1 || Math.abs(o.y - c.y) > 1;
  }

  function isResized(s: DetectedSection): boolean {
    const o = s.originalRect, c = s.currentRect;
  ...
  $effect(() => {
    ...
    for (const id of firstActionRef.keys()) {
      if (!sections.some(s => s.id === id)) firstActionRef.delete(id);
    }
  });
  ...
  const visibleSections = $derived(
    sections.filter(s => {
      try {
        // scrollY referenced to recompute on scroll
        scrollY;
  ...
  const changedSections = $derived(visibleSections.filter(s => hasChanged(s)));
  const unchangedSections = $derived(visibleSections.filter(s => !hasChanged(s)));

  const currentChangedIds = $derived(new Set(changedSections.map(s => s.id)));
  const changedKey = $derived([...currentChangedIds].sort().join(","));

  // Clean up seenGhostIds / keep last known positions in sync with derived data
  $effect(() => {
  ...
  $effect(() => {
    // deps: changedKey, sections
    changedKey; sections;
    const prev = prevChangedIdsRef;
    prevChangedIdsRef = currentChangedIds;
    const exitingMap = new Map<string, { orig: Rect; target: Rect; isFixed?: boolean }>();
    ...
    if (exitingMap.size > 0) {
      const next = new Map(exitingConnectors);
      for (const [id, data] of exitingMap) next.set(id, data);
      exitingConnectors = next;
      const timer = originalSetTimeout(() => {
        const n2 = new Map(exitingConnectors);
        for (const id of exitingMap.keys()) n2.delete(id);
        exitingConnectors = n2;
      }, 250);
      return () => clearTimeout(timer);
    }
  });

  // Connector sections (committed changes + live drags + exiting)
  type ConnectorSection = { id: string; orig: Rect; target: Rect; isFixed?: boolean; isSelected: boolean; isExiting?: boolean };
  const connectorSections = $derived.by<ConnectorSection[]>(() => {
    if (blankCanvas) return [];
    const result: ConnectorSection[] = [];
    for (const s of changedSections) {
  ...
  // Note popup positioning (derived from editingId)
  type EditState = { section: DetectedSection; style: string } | null;
  const editState = $derived.by<EditState>(() => {
    ...
    return { section: es, style };
  });

  // Resolve a note (and remember last value for fade-out)
  function noteFor(section: DetectedSection): string {
    if (section.note) lastNoteTextRef.set(section.id, section.note);
    return section.note || lastNoteTextRef.get(section.id) || "";
  }

  function handleClass(dir: HandleDir): string {
    const key = `handle${dir.charAt(0).toUpperCase()}${dir.slice(1)}` as keyof typeof styles;
    return `${styles.handle} ${styles[key]}`;
  }

  function ghostBadgeMoved(section: DetectedSection): string {
    const first = firstActionRef.get(section.id);
    return first === "resize" ? "Resize" : "Move";
  }
  function ghostBadgeExtra(section: DetectedSection): string {
    const first = firstActionRef.get(section.id);
    return first === "resize" ? "Move" : "Resize";
  }

  // Mark a ghost as seen and report whether it was new (for ghostEnter animation)
  function consumeNewGhost(id: string): boolean {
    const isNew = !seenGhostIdsRef.has(id);
    if (isNew) seenGhostIdsRef.add(id);
    return isNew;
  }
</script>

<div
  class={`${styles.rearrangeOverlay} ${!isDarkMode ? styles.light : ""} ${exiting ? styles.overlayExiting : ""}${extraClassName ? ` ${extraClassName}` : ""}`}
...
>
  ...
  {#each unchangedSections as section (section.id)}
    ...
    <div
      ...
      class={`${styles.sectionOutline} ${isSelected ? styles.selected : ""} ${exitingAll || exiting || exitingIds.has(section.id) ? styles.exiting : ""}`}
      style="left: {rect.x}px; top: {screenY}px; width: {rect.width}px; height: {rect.height}px; border-color: {SECTION_COLOR.border}; background-color: {SECTION_COLOR.bg};{outlinesReady ? '' : ' opacity: 0; animation: none; transition: none;'}"
      onmousedown={(e) => handleOutlineMouseDown(e, section.id)}
      ondblclick={() => handleDoubleClick(section.id)}
      role="presentation"
    >
      <span class={styles.sectionLabel} style="background-color: {SECTION_COLOR.pill}">{section.label}</span>
      <span class={`${styles.sectionAnnotation} ${section.note ? styles.annotationVisible : ""}`}>{noteFor(section)}</span>
      <span class={styles.sectionDimensions}>{Math.round(rect.width)} &times; {Math.round(rect.height)}</span>
      <div
      ...
      {#each HANDLES as dir (dir)}
        <div class={handleClass(dir)} onmousedown={(e) => handleResizeMouseDown(e, section.id, dir)} role="presentation"></div>
      {/each}
    </div>
  {/each}
  ...
  {#each changedSections as section (section.id)}
    ...
    {#if !(blankCanvas && settled)}
      {@const isNewGhost = consumeNewGhost(section.id)}
      <div
        data-rearrange-section={section.id}
        class={`${styles.ghostOutline} ${isSelected ? styles.selected : ""} ${exitingAll || exiting || exitingIds.has(section.id) ? styles.exiting : ""}`}
      ...
      >
        ...
        <span class={styles.ghostBadge}>
          {#if moved && resized}
            Suggested {ghostBadgeMoved(section)} <span class={styles.ghostBadgeExtra}>&amp; {ghostBadgeExtra(section)}</span>
          {:else}
            Suggested {resized ? "Resize" : "Move"}
          {/if}
        </span>
      </div>
    {/if}
...
{#if !blankCanvas && connectorSections.length > 0}
  <svg class={`${styles.connectorSvg} ${exitingAll || exiting ? styles.connectorExiting : ""}`}>
    {#each connectorSections as conn (conn.id)}
      {@const ox = conn.orig.x + conn.orig.width / 2}
      {@const oy = (conn.isFixed ? conn.orig.y : conn.orig.y - scrollY) + conn.orig.height / 2}
      {@const cx = conn.target.x + conn.target.width / 2}
      {@const cy = (conn.isFixed ? conn.target.y : conn.target.y - scrollY) + conn.target.height / 2}
      {@const ddx = cx - ox}
      {@const ddy = cy - oy}
      ...
      {#if dist >= 2}
        ...
        {@const isDragging = dragPositions.has(conn.id)}
        {@const baseOpacity = isDragging || conn.isSelected ? 1 : 0.4}
        {@const dotBaseOpacity = isDragging || conn.isSelected ? 1 : 0.5}
        <g class={conn.isExiting ? styles.connectorExiting : ""}>
          <path
...
{#if editState}
  <AnnotationPopupCSS
    element={editState.section.label}
    placeholder="Add a note about this section"
    initialValue={editState.section.note ?? ""}
...
{#if sizeIndicator}
  <div class={styles.sizeIndicator} style="left: {sizeIndicator.x}px; top: {sizeIndicator.y}px;" data-feedback-toolbar>
    {sizeIndicator.text}
  </div>
{/if}

<!-- Snap alignment guides -->
{#each snapGuides as g, i (`${g.axis}-${g.pos}-${i}`)}
  <div
