...
interface FreezeState {
  frozen: boolean;
  installed: boolean;
  // Browser timer signatures (return `number`, not NodeJS.Timeout). Typed
  // explicitly so an ambient @types/node in the workspace can't widen these.
  origSetTimeout: (handler: TimerHandler, timeout?: number, ...args: any[]) => number;
  origSetInterval: (handler: TimerHandler, timeout?: number, ...args: any[]) => number;
